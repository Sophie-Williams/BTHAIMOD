...
public:
	...
	/** Returns true (defensive) or false (offensive) depending on
	 * how the agent moves. */
	bool getDefensive();

	/** Checks if the unit is allowed to switch target. Needed to avoid
	 * target switch occilation. */
	bool canSwitchTarget();

	/** Sets that this unit has switched target. */
