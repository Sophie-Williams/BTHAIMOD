...
void StructureAgent::computeActions()
{
	if (isAlive())
	{
		...
		{
			if (Broodwar->canMake(unit, UnitTypes::Zerg_Greater_Spire) && AgentManager::getInstance()->countNoFinishedUnits(UnitTypes::Zerg_Hive) > 0)
			{
...
void StructureAgent::printInfo()
{
	...
	if (isOfType(UnitTypes::Terran_Bunker))
	{
		...
		if (sq != NULL)
		{
			...
			{
				Broodwar->drawTextMap(sx+2,h-60+15*no,"%s \x11(%d)", format(agents.at(i)->getUnitType().getName()).c_str(), agents.at(i)->getUnitID());
				no++;
			}
		}
