...
{
	stringstream ss;
	ss << "bwapi-data\\AI\\BTHAI\\mapdata\\";
	ss << Broodwar->mapHash();
	ss << ".txt";
...
/*double BuildingPlacer::getDistTo(TilePosition pos)
{
	...
	//Test
	BWTA::Region* cRegion = getRegion(pos);
	if (cRegion != NULL)
	{
		for(set<BWTA::Region*>::const_iterator i=cRegion->getReachableRegions().begin();i!=cRegion->getReachableRegions().end();i++)
		{
			if (isOccupied((*i)))
			{
				dist = dist * 0.7;
