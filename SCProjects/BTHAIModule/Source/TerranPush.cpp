...
TerranPush::TerranPush()
{
	...
	buildplan.push_back(BuildplanEntry(UnitTypes::Terran_Factory, 15));
	buildplan.push_back(BuildplanEntry(UnitTypes::Terran_Supply_Depot, 16));
	buildplan.push_back(BuildplanEntry(UnitTypes::Terran_Command_Center, 17));
	...
	buildplan.push_back(BuildplanEntry(UnitTypes::Terran_Academy, 24));
	buildplan.push_back(BuildplanEntry(UnitTypes::Terran_Armory, 28));
	buildplan.push_back(BuildplanEntry(UpgradeTypes::U_238_Shells, 28));
...
void TerranPush::computeActions()
{
	computeActionsBase();
	...
	if (stage == 2 && AgentManager::getInstance()->countNoBases() >= 2)
	{
