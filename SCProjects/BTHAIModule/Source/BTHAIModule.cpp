...
void BTHAIModule::onEnd(bool isWinner) 
{
	...
	//Default Logic
	StrategySelector::getInstance()->addResult(isWinner);
	StrategySelector::getInstance()->saveStats();

	//Meta-Level Reinforcement Learning Logic
	StrategySelector::getInstance()->addLearnedResult(isWinner);
	StrategySelector::getInstance()->saveLearnedMatchs();
